...
// e backups antigos ainda abrem com o codigo atual. Mesmo formato do app
// mobile (o cofre e compativel byte-a-byte entre as duas plataformas).
...
describe("vaultCrypto (desktop)", () => {
  ...
  it("gera o mesmo formato de envelope que o app mobile (compatibilidade byte-a-byte)", async () => {
    // Nao compara com o output real do mobile aqui (esse teste roda so
    // no processo do desktop, sem acesso a react-native-quick-crypto),
    // mas confirma que a FORMA do envelope - os campos e tipos que
    // precisam bater pro cofre ser compativel entre os dois apps - e a
    // mesma descrita em src/services/vaultCrypto.js (app mobile).
    const envelope = await encryptVaultItems(sampleItems, "secret");
    expect(Object.keys(envelope).sort()).toEqual(
      ["type", "version", "alg", "kdf", "iv", "ciphertext", "authTag"].sort(),
    );
    expect(Object.keys(envelope.kdf).sort()).toEqual(
      ["name", "iterations", "salt"].sort(),
    );
  });
});
